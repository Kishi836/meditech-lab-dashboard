...
"Meditech Lab" (Module 2 of *Medical Informatics · UE26MT324*). Four modules
...
raw HL7 to a local archive dir). Every row the pipeline writes is tagged with
an `ENC-HL7-` `enc_id` prefix so it never pollutes the seed data, and
`Reset` cleans it all up.
...
| `GET  /api/pipeline/reset`                 | delete pipeline-created rows + archive   |
